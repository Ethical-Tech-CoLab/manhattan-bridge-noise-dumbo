...
--------------------
The usage dashboard in this repository reads `~/.copilot/session-store.db`,
which is local to one machine. This project was also worked on from a second
machine, in four sibling repositories. That work is real, it cost real money,
and it is invisible to a dashboard that can only see one store. There is no
network API to ask for it: the store is a local SQLite file and nothing
uploads it.

So this script runs on the OTHER machine and writes a small JSON file per
session. Those files are committed to `usage/contrib/` and the generator
merges them.
...
said. That is deliberate: this repository publishes its own cost data, and it
would be careless to build an exporter that quietly carried conversation text
along with it.
    ...
    for r in rows:
        ...
        if not raw:
            die(
                ...
                "figure that reconciles - see usage/README.md trap 2." % r["id"]
            )
        chans, nano = {}, 0
...
def main():
    ...
    try:
        ...
        print("Copy them into usage/contrib/ in manhattan-bridge-noise-dumbo "
              "and re-run usage/build_usage_data.py.")
    finally:
        con.close()
