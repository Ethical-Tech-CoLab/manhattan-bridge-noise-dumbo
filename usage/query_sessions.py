...
needs `usage/contrib/` files to see past one machine. This script answers the
...
`sessions.repository` / `sessions.cwd` say which project each row belongs to.
    ...
    python usage/query_sessions.py                 # last 7 days, by session
    python usage/query_sessions.py --days 30       # a longer window
    python usage/query_sessions.py --all           # everything retained
    python usage/query_sessions.py --by day        # calendar days, local zone
    python usage/query_sessions.py --by model
    python usage/query_sessions.py --by repo       # sessions folded by project
    python usage/query_sessions.py --json          # for piping somewhere
...
WHY THIS IS NOT ONE LINE OF SQL. Two of the obvious aggregations are wrong:
  ...
  main agent, not after it, so their durations overlap. On the session that
  wrote this repository the naive sum reads 16.79 h against a true union of
...
Days are cut on LOCAL midnight, not UTC: 19 per cent of this project's
requests land between 00:00 and 04:00 UTC, which is the previous evening in
New York, and a UTC cut misattributed one day by a factor of five.
